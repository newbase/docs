...
const OUTPUT_DIR = join(process.cwd(), 'screenshots', 'admin-all');
const WAIT_NAV = 2200;
...
async function main() {
  ...
  try {
    ...
    // ---------- 1. 대시보드 ----------
    console.log('[1] 대시보드');
    await page.goto(`${BASE_URL}/admin/dashboard`, { waitUntil: 'networkidle', timeout: 20000 });
    ...
    console.log('\n[2] 기관고객 관리 (목록)');
    await page.goto(`${BASE_URL}/admin/organizations`, { waitUntil: 'networkidle', timeout: 20000 });
    await page.waitForTimeout(WAIT_NAV);
    ...
    await page.waitForTimeout(500);
    await page.goto(`${BASE_URL}/admin/organizations/${TARGET_ORG_ID}`, { waitUntil: 'networkidle', timeout: 20000 });
    await page.waitForTimeout(WAIT_NAV);
    ...
    console.log('\n[3] 클래스 관리 (목록)');
    await page.goto(`${BASE_URL}/admin/class-management`, { waitUntil: 'networkidle', timeout: 20000 });
    await page.waitForTimeout(WAIT_NAV);
      ...
      if (await titleCell.isVisible().catch(() => false)) {
        ...
        if (await openPmBtn.isVisible().catch(() => false)) {
          ...
          if (page.url().includes('/results/')) {
            ...
            await page.goto(openDetailUrl, { waitUntil: 'networkidle', timeout: 20000 });
            await page.waitForTimeout(WAIT_NAV);
        ...
        if (await openCloneBtn.isVisible().catch(() => false)) {
          await openCloneBtn.click();
          await page.waitForURL(/\/open-class\/create/, { timeout: 10000 });
          await page.waitForTimeout(WAIT_NAV);
      ...
      if (await titleCell.isVisible().catch(() => false)) {
        ...
        if (await pmBtn.isVisible().catch(() => false)) {
          ...
          if (page.url().includes('/results/')) {
            ...
            await page.goto(classDetailUrl, { waitUntil: 'networkidle', timeout: 20000 });
            await page.waitForTimeout(WAIT_NAV);
          } else if (await page.locator('[role="dialog"]').count() > 0) {
    ...
    // ---------- 4. 프로덕트 CRUD ----------
    console.log('\n[4] 프로덕트 관리');
    await page.goto(`${BASE_URL}/admin/product-management`, { waitUntil: 'networkidle', timeout: 20000 });
    await page.waitForTimeout(WAIT_NAV);
    ...
    if (await productCreateBtn.isVisible().catch(() => false)) {
      ...
      await page.goto(`${BASE_URL}/admin/product/create`, { waitUntil: 'networkidle' });
    }
    ...
    // ---------- 7. 주문 관리 + 주문 등록 ----------
    console.log('\n[7] 주문 관리');
    await page.goto(`${BASE_URL}/admin/order-management`, { waitUntil: 'networkidle', timeout: 20000 });
    await page.waitForTimeout(WAIT_NAV);
    ...
    await page.goto(`${BASE_URL}/admin/assets/events`, { waitUntil: 'networkidle', timeout: 20000 });
    await page.waitForTimeout(WAIT_NAV);
