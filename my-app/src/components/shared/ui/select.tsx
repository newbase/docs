...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-[38px] w-full items-center justify-between rounded-md border border-input bg-white px-3 py-2 text-sm ring-offset-background data-[placeholder]:text-muted-foreground hover:border-blue-500 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1",
      className
    )}
    {...props}
  >
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex w-full cursor-default select-none items-center hover:text-blue-500 rounded-sm py-2 pl-8 pr-2 text-sm outline-none focus:bg-accent focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]",
      className
    )}
    {...props}
  >
          ...
          <div className="absolute z-[1100] mt-1 max-h-60 w-full overflow-auto rounded-md border border-gray-200 bg-white py-1 shadow-lg ring-2 ring-black ring-opacity-5 focus:outline-none">
            {filteredOptions.map((option) => (
              <div
                key={option.value}
                className={cn(
                  "relative cursor-default select-none py-2 pl-3 pr-9 text-sm transition-colors hover:bg-blue-50 hover:text-blue-500",
                  value === option.value ? "bg-blue-50 text-blue-500" : "text-gray-900"
                )}
                onMouseDown={(e) => {
...
// Simple Select component (similar to Input component)
...
const SimpleSelect = React.forwardRef<HTMLSelectElement, SimpleSelectProps>(
  ({ className, label, error, children, wrapperClassName, ...props }, ref) => {
    return (
      <div className={cn("w-full", wrapperClassName)}>
        ...
        <div className="relative">
          <select
            className={cn(
              "flex h-[38px] w-full rounded-md border border-input bg-white pl-3 pr-10 py-2 text-sm ring-offset-background transition-all appearance-none",
