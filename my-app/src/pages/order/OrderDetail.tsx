...
/** 주문유형별 품목 한 줄 (OrderCreate 품목 행과 동일 구조) */
...
export const MOCK_ORDERS: OrderDetailRow[] = [
    { id: 'ORD-2025-001', ordererName: '서울대학교병원', type: '구독', currency: 'KRW', amount: 5500000, cost: 4000000, expectedProfit: 1000000, actualProfit: 1000000, salesChannel: '본사', status: '상담', orderNote: '', items: [{ id: 'i1', productName: '메디컬 에듀 구독', optionName: 'Pro', period: 12, periodUnit: 'MONTH', startDate: '2025-03-01', quantity: 50, unitPrice: 110000, amount: 5500000 }], createdAt: '2025-02-10 14:30', updatedAt: '2025-02-10 14:30' },
    { id: 'ORD-2025-002', ordererName: '연세대학교병원', type: '장비/물품', currency: 'KRW', amount: 3300000, cost: 2500000, expectedProfit: 500000, actualProfit: 0, salesChannel: '본사', status: '진행', items: [{ id: 'i1', productName: 'VR 헤드셋 패키지', optionName: '기본', startDate: '2025-02-15', quantity: 30, unitPrice: 100000, costPrice: 83333, amount: 3000000 }, { id: 'i2', productName: '설치 키트', optionName: '기본', startDate: '2025-02-20', quantity: 3, unitPrice: 100000, costPrice: 66667, amount: 300000 }], createdAt: '2025-02-10 11:20', updatedAt: '2025-02-11 09:00' },
    { id: 'ORD-2025-003', ordererName: '삼성서울병원', type: '구독', currency: 'KRW', amount: 11000000, cost: 8000000, expectedProfit: 2000000, actualProfit: 0, salesChannel: '메디컬에듀 판매처', status: '진행', items: [{ id: 'i1', productName: '메디컬 에듀 구독', optionName: 'Enterprise', period: 12, periodUnit: 'MONTH', startDate: '2025-03-15', quantity: 100, unitPrice: 110000, amount: 11000000 }], createdAt: '2025-02-11 09:15', updatedAt: '2025-02-12 10:00' },
    { id: 'ORD-2025-004', ordererName: '가톨릭대학교 서울성모병원', type: '커스텀 서비스', currency: 'KRW', amount: 2750000, cost: 1500000, expectedProfit: 800000, actualProfit: 750000, salesChannel: '본사', status: '완료', items: [{ id: 'i1', productName: '맞춤 시뮬레이션 코스', optionName: '기본', period: 6, periodUnit: 'MONTH', startDate: '2025-02-20', quantity: 25, unitPrice: 110000, amount: 2750000 }], createdAt: '2025-02-11 16:45', updatedAt: '2025-02-13 14:00' },
    { id: 'ORD-2025-005', ordererName: '분당서울대병원', type: '오픈클래스', currency: 'KRW', amount: 2200000, cost: 1200000, expectedProfit: 700000, actualProfit: 680000, salesChannel: '본사', status: '완료', items: [{ id: 'i1', productName: '응급의료 오픈클래스', period: 30, periodUnit: 'DAY', startDate: '2025-02-25', quantity: 20, unitPrice: 110000, amount: 2200000 }], createdAt: '2025-02-12 10:00', updatedAt: '2025-02-14 11:00' },
    { id: 'ORD-2025-006', ordererName: '강남세브란스병원', type: '장비/물품', currency: 'KRW', amount: 4400000, cost: 3200000, expectedProfit: 900000, actualProfit: -50000, salesChannel: '헬스케어솔루션', status: '환불완료', items: [{ id: 'i1', productName: 'VR 헤드셋 패키지', optionName: '기본', startDate: '2025-02-18', quantity: 40, unitPrice: 110000, costPrice: 80000, amount: 4400000 }], createdAt: '2025-02-09 15:30', updatedAt: '2025-02-15 16:00' },
    { id: 'ORD-2025-007', ordererName: '고려대학교안암병원', type: '구독', currency: 'KRW', amount: 1100000, cost: 800000, expectedProfit: 200000, actualProfit: 0, salesChannel: '본사', status: '대기', items: [{ id: 'i1', productName: '메디컬 에듀 구독', optionName: 'Basic', period: 12, periodUnit: 'MONTH', startDate: '2025-04-01', quantity: 10, unitPrice: 110000, amount: 1100000 }], createdAt: '2025-02-08 13:20', updatedAt: '2025-02-08 13:20' },
];
            ...
            <div className="space-y-6 mt-6">
                <section className="bg-white rounded-lg border border-gray-400 shadow-sm space-y-4 px-6 py-4">
                    ...
                    <div className="grid grid-cols-4 gap-x-8 gap-y-0">
                        ...
                        <div className="flex items-center gap-3 min-w-0">
                            <span className="text-sm font-medium shrink-0 w-20 text-gray-500">주문유형</span>
                    ...
                    {/* 품목 정보 (주문유형별 상세: 구독·장비/물품·커스텀·오픈클래스) */}
                    {order.items && order.items.length > 0 && (() => {
                        const type = order.type;
                        const isSubscription = type === '구독';
                        const isProduct = type === '장비/물품';
                        const isCustom = type === '커스텀 서비스';
                        const isOpenClass = type === '오픈클래스';
                        const showOption = isSubscription; // 구독 플랜만 표시
                        const showSubscriptionPeriod = isSubscription || isCustom;
                        const showInvitePeriod = isOpenClass;
                                ...
                                <Table className="w-full">
                                    <TableHeader className="bg-brand-50">
                                        <TableRow>
                                            ...
                                            {showOption && <TableHead className={thClass + ' min-w-[6rem]'}>{'구독플랜'}</TableHead>}
                                            {showSubscriptionPeriod && <TableHead className={thClass + ' min-w-[6rem]'}>{'구독기간'}</TableHead>}
                                            {showInvitePeriod && <TableHead className={thClass + ' min-w-[6rem]'}>{'초대유효기간'}</TableHead>}
                                        ...
                                        {order.items.map((item) => {
                                            ...
                                            return (
                                                <TableRow key={item.id} className="text-sm">
                                                    ...
                                                    {showSubscriptionPeriod && <TableCell className={'px-3 py-2 ' + colClass}>{periodLabel}</TableCell>}
                                                    {showInvitePeriod && <TableCell className={'px-3 py-2 ' + colClass}>{periodLabel}</TableCell>}
