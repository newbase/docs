...
import { ScenarioDetail, scenarioDetails } from '../../data/scenarioDetails';
...
export default function ClassEdit(): React.ReactElement {
    ...
    useEffect(() => {
        ...
        const classData = (classesData as unknown as Record<string, ClassItem>)[id];
        if (!classData) {
            alert('클래스를 찾을 수 없습니다.');
            navigate(classManagementPath);
            return;
        }
        ...
        setMaxParticipants(classData.maxParticipants || 30);
        setIsPublic((classData as ClassItem & { isPublic?: boolean }).isPublic ?? true);
        setPassword(classData.password || '');

        // Load admin-specific fields
        if (isAdmin) {
            setSaleStatus(classData.saleStatus || (classData.isForSale ? 'available' : 'unavailable'));
            if (classData.organizationId) {
                setSelectedOrganizationId(classData.organizationId.toString());
            }
            if (classData.price !== undefined) {
                setPrice(classData.price);
            }
            if (classData.discountPrice !== undefined) {
                setDiscountPrice(classData.discountPrice);
            }
        }
        ...
        if (classData.thumbnail) {
            // Check if it looks like a custom upload or scenario image (simplified check)
            // For now, assume custom if it matches internal path structure or just set as custom
            setCustomThumbnail(classData.thumbnail);
            setThumbnailMode('custom');
        }

        // Map curriculum
        const items: CurriculumItem[] = classData.curriculum.map((item, index) => {
            // Try to find in scenarioDetails
            const detail = scenarioDetails[item.id];

            if (item.type === 'video') {
                // Mock video object since we don't have a video database yet
                const videoData: VideoLecture = {
                    ...
                    author: {
                        ...
                        type: 'institution' // Default for existing data
                    },
                ...
                // Scenario
                const scenarioData: ScenarioDetail = detail || {
                    id: item.id,
                    title: item.name,
                    duration: item.duration,
                    platform: (item.platform === 'both' ? 'VR' : item.platform) as 'VR' | 'Mobile' | 'Web',
                    ContributedBy: 'Unknown',
                    patient: classData.patientType || 'Unknown',
                    map: 'Hospital',
    ...
    const validateForm = (): boolean => {
        ...
        if (shouldValidateMaxParticipants) {
            if (!maxParticipants || maxParticipants < 1) {
                ...
                newErrors.maxParticipants = `등록 가능한 사용자 수(${licenseInfo.count}개)를 초과할 수 없습니다.`;
            }
        }
