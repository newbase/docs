...
import { Plus, GripVertical, GraduationCap, Pencil, Check, ChevronDown, ChevronUp } from 'lucide-react';
...
import { CurriculumItem, CurriculumItemType, VideoLecture } from '../../types/curriculum';
...
export default function CurriculumCreate({
...
}: CurriculumCreateProps): React.ReactElement {
    ...
    const handleAddScenarios = (scenarios: ScenarioDetail[]) => {
        if (!scenarios?.length) return;
        const newItems: CurriculumItem[] = scenarios.map((detail) => ({
            id: `scenario-${detail.id}-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`,
            type: 'scenario' as CurriculumItemType,
            data: detail
        }));
        onItemsChange([...(items || []), ...newItems]);
    };

    const selectedScenarioIds = useMemo(() => (items || [])
        .filter((i): i is CurriculumItem & { type: 'scenario' } => i?.type === 'scenario')
        .map((i) => (i.data as ScenarioDetail)?.id)
        .filter((id): id is number => id != null), [items]);
                ...
                <>
                    <div className="flex items-center justify-between py-2">
                        ...
                        <div className="flex gap-2">
                            {effectiveEditMode && (
                                <>
                                    <Button variant="outline" size="sm" onClick={() => setIsScenarioModalOpen(true)}>
                    ...
                    {effectiveEditMode ? (
                        <DragDropContext onDragEnd={handleDragEnd}>
                            <Droppable droppableId="curriculum-create">
                                {(provided, snapshot) => (
                                    <div
                    ...
                    {effectiveEditMode && (
                        <p className="text-sm text-gray-500 mt-4">
                            💡 세션 선택, 시나리오 추가, 동영상 추가, 드래그로 순서 변경이 가능합니다.
                        </p>
                    )}
                </>
            )}
